...
public class GeneratorJob extends NutchTool implements Tool {
	public static final String COUNTER_GROUP_GENERATE = "Generator";
	...
	public static final String VISITED_COUNT = "visited.count";
	...
	public static class SelectorEntry implements WritableComparable<SelectorEntry> {
		...
		public int compareTo(SelectorEntry se) {
			if (se.score > score) return 1;
			else if (se.score == score) return url.compareTo(se.url);
			return -1;
		}
		...
		public String toString() {
			return "SelectorEntry{" + "url='" + url + '\'' + ", score=" + score + '}';
		}
	...
	public Map<String, Object> run(Map<String, Object> args) throws Exception {
		LOG.debug(">>> Generator arguments {}", args);
		...
		if (sitemap != null) getConf().setBoolean(GENERATOR_SITEMAP, sitemap);
		...
		long generateCount = currentJob.getCounters().findCounter(COUNTER_GROUP_GENERATE, GeneratorReducer.COUNTER_GENERATE_MARK).getValue();
		long skippedCount = currentJob.getCounters().findCounter(COUNTER_GROUP_GENERATE, GeneratorMapper.COUNTER_SKIPPED).getValue();
		long visitedCount = currentJob.getCounters().findCounter(COUNTER_GROUP_GENERATE, GeneratorMapper.COUNTER_VISITED).getValue();

		results.put(GENERATE_COUNT, generateCount);
		results.put(SKIPPED_COUNT, skippedCount);
		results.put(VISITED_COUNT, visitedCount);
	 ...
	 */
	public String generate(long topN, long curTime, boolean filter, boolean norm, boolean sitemap) throws Exception {
		...
		Map<String, Object> results = run(ToolUtil.toArgMap(Nutch.ARG_TOPN, topN, Nutch.ARG_CURTIME, curTime, Nutch.ARG_FILTER, filter, Nutch.ARG_NORMALIZE,
				norm, Nutch.ARG_BATCH, batchId, Nutch.ARG_SITEMAP, sitemap));
		if (batchId == null) {
		...
		long skippedCount = (Long) results.get(SKIPPED_COUNT);
		long visitedCount = (Long) results.get(VISITED_COUNT);
		LOG.info("GeneratorJob: finished at {}, time elapsed: {}", sdf.format(finish), TimingUtil.elapsedTime(start, finish));
		LOG.info("GeneratorJob: generated batch id: {} containing {} URLs [ visited:{} / skipped:{} ]", batchId, generateCount, visitedCount, skippedCount);
		if (generateCount == 0) {
			return null;
		}
		return batchId;
	...
	public int run(String[] args) throws Exception {
		if (args.length <= 0) {
			System.out.println("Usage: GeneratorJob [-topN N] [-crawlId id] [-noFilter] [-noNorm] [-adddays numDays] [-sitemap]");
			...
			System.out.println("    -sitemap       - generate only sitemap url, default false");
