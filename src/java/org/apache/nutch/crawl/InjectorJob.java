...
public class InjectorJob extends NutchTool implements Tool {
	...
	public static class UrlMapper extends Mapper<LongWritable, Text, String, WebPage> {
				...
				for (int s = 1; s < splits.length; s++) {
					...
					if (splits[s].indexOf("sitemaps:") > -1) {
						...
						for (int i = 1; i < sitemaps.length; i++) {
							...
							write(sitemapUrl, context, customInterval, customScore, new HashMap<String, String>(), InjectType.SITEMAP_INJECT);
						}
						continue;
		...
		private void write(String url, Context context, Integer customInterval, Float customScore, Map<String, String> metadata, InjectType injectType) throws
