...
public class FetcherJob extends NutchTool implements Tool {
	...
	public Map<String, Object> run(Map<String, Object> args) throws Exception {
		...
		StorageUtils.initMapperJob(currentJob, fields, IntWritable.class, FetchEntry.class, FetcherMapper.class, FetchEntryPartitioner.class, batchIdFilter,
				false);
	 ...
	 * Run fetcher.
	 *
	 * @param batchId      batchId (obtained from Generator) or null to fetch all generated
	 *                     fetchlists
	 * @param threads      number of threads per map task
	 * @param shouldResume
	 * @param numTasks     number of fetching tasks (reducers). If set to < 1 then use the
	 ...
	 * @param numTasks     number of fetching tasks (reducers). If set to < 1 then use the
	 *                     default, which is mapred.map.tasks.
	 * @param stmDetect    If set true, sitemap detection is run.
	 * @param sitemap      If set true, only sitemap files is fetched, If set false, only
	 *                     normal urls is fetched.
	 * @return 0 on success
	 * @throws Exception
	 */
	public int fetch(String batchId, int threads, boolean shouldResume, int numTasks, boolean stmDetect, boolean sitemap) throws Exception {
		...
		run(ToolUtil.toArgMap(Nutch.ARG_BATCH, batchId, Nutch.ARG_THREADS, threads, Nutch.ARG_RESUME, shouldResume, Nutch.ARG_NUMTASKS, numTasks, Nutch
				.ARG_SITEMAP_DETECT, stmDetect, Nutch.ARG_SITEMAP, sitemap));
	...
	public int run(String[] args) throws Exception {
		...
		String usage = "Usage: FetcherJob (<batchId> | -all) [-crawlId <id>] " + "[-threads N] \n \t \t  [-resume] [-numTasks N]\n" + "    <batchId>     - " +
				"crawl identifier returned by Generator, or -all for all \n \t \t    generated batchId-s\n" + "    -crawlId <id> - the id to prefix the " +
				"schemas to operate on, \n \t \t    (default: storage.crawl.id)\n" + "    -threads N    - number of fetching threads per task\n" + "    " +
				"-resume       - resume interrupted job\n" + "    -numTasks N   - if N > 0 then use this many reduce tasks for fetching \n \t \t    (default:"
				+ " mapred.map.tasks)" + "    -sitemap      - only sitemap files are fetched, defaults to false" + "    -stmDetect    - sitemap files are " +
				"detected from robot.txt file";
		...
		int fetchcode = fetch(batchId, threads, shouldResume, numTasks, stmRobot, sitemap); // run the Fetcher
